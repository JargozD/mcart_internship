...
<? if (!empty($arResult)) : ?>
	...
	<div class="menu-block popup-wrap">
		...
		<div class="menu popup-block">
			<ul class="">
				<li class="main-page">
					<a href="/exam-e3/"><? echo GetMessage("MAIN_PAGE") ?></a>
				</li>
				...
				foreach ($arResult as $arItem) : ?>
					...
					<? if ($arItem["IS_PARENT"] && $arItem["PERMISSION"] > "D") : ?>
						...
						<? if ($arItem["DEPTH_LEVEL"] == 1) : ?>
							...
							<li><a href="<?= $arItem["LINK"] ?>" class="<? if ($arItem["SELECTED"]) : ?>root-item-selected<? else : ?>root-item<? endif ?> <?= $sClassStyle ?>"><?= $arItem["TEXT"] ?></a>
								<ul>
								<? else : ?>
